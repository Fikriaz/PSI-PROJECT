...
if ($result->num_rows > 0) {
    ...
    $totalKeuntunganTotal = $row["total_keuntungan"];
} else {
    $totalKeuntunganTotal = 0;
}

//total bbm terjual hari 
$queryBbmTerjual = "SELECT SUM(jumlah_liters) AS total_bbm_terjual FROM transaksi WHERE DATE(tanggal_transaksi) = CURDATE()";
$resultBbmTerjual = $conn->query($queryBbmTerjual);
$rowBbmTerjual = $resultBbmTerjual->fetch_assoc();
$totalBbmTerjualHariIni = $rowBbmTerjual["total_bbm_terjual"];

//total penjualan hari ini 
$queryPenjualan = "SELECT SUM(keuntungan) AS total_penjualan FROM transaksi WHERE DATE(tanggal_transaksi) = CURDATE()";
...
// Mendapatkan penjualan paling sedikit dalam liter
$queryPenjualanPalingSedikit = "SELECT jenis_bbm.nama AS penjualan_paling_sedikit, SUM(transaksi.jumlah_liters) AS total_penjualan_paling_sedikit
                                FROM transaksi
                                JOIN jenis_bbm ON transaksi.jenis_bbm_id = jenis_bbm.id
                                GROUP BY transaksi.jenis_bbm_id
                                ORDER BY total_penjualan_paling_sedikit ASC
                                LIMIT 1";
$resultPenjualanPalingSedikit = $conn->query($queryPenjualanPalingSedikit);
$rowPenjualanPalingSedikit = $resultPenjualanPalingSedikit->fetch_assoc();
$penjualanPalingSedikit = $rowPenjualanPalingSedikit["penjualan_paling_sedikit"];
$totalPenjualanPalingSedikit = $rowPenjualanPalingSedikit["total_penjualan_paling_sedikit"];

// Mendapatkan penjualan paling banyak dalam liter
$queryPenjualanPalingBanyak = "SELECT jenis_bbm.nama AS penjualan_paling_banyak, SUM(transaksi.jumlah_liters) AS total_penjualan_paling_banyak
                              FROM transaksi
                              JOIN jenis_bbm ON transaksi.jenis_bbm_id = jenis_bbm.id
                              GROUP BY transaksi.jenis_bbm_id
                              ORDER BY total_penjualan_paling_banyak DESC
                              LIMIT 1";
$resultPenjualanPalingBanyak = $conn->query($queryPenjualanPalingBanyak);
$rowPenjualanPalingBanyak = $resultPenjualanPalingBanyak->fetch_assoc();
$penjualanPalingBanyak = $rowPenjualanPalingBanyak["penjualan_paling_banyak"];
$totalPenjualanPalingBanyak = $rowPenjualanPalingBanyak["total_penjualan_paling_banyak"];


// Mendapatkan jenis BBM paling favorit
$queryFavorit = "SELECT jenis_bbm.nama AS jenis_bbm_favorit, SUM(transaksi.jumlah_liters) AS total_liters_terjual
                FROM transaksi
                JOIN jenis_bbm ON transaksi.jenis_bbm_id = jenis_bbm.id
                GROUP BY transaksi.jenis_bbm_id
                ORDER BY total_liters_terjual DESC
                LIMIT 1";
$resultFavorit = $conn->query($queryFavorit);
$rowFavorit = $resultFavorit->fetch_assoc();
$jenisBBMFavorit = $rowFavorit["jenis_bbm_favorit"];
$totalLitersTerjual = $rowFavorit["total_liters_terjual"];
...
<body class=" m-0  text-base antialiased font-normal  leading-default text-slate-500 font-[Poppins]">
    ...
    <main class="lg:ml-80  ">
        <div
            class="flex items-center justify-between  lg:w-full sm:w-full text-black w-full  py-1 mx-auto flex-wrap-inherit">
            <nav>
                <!-- navbar -->
                <ol class="flex flex-wrap pt-1  bg-transparent rounded-lg sm:mr-16">
                    ...
                    <li class="text-sm  capitalize leading-normal text-black before:float-left before:pr-2
           before:text-black before:content-['/']" aria-current="page">Dashboard</li>
                </ol>
                <h6 class="mb-0 font-semibold text-black capitalize">Dashboard</h6>
            </nav>
                    ...
                    <div class="flex items-center justify-between">
                        <div class="widget-label">
                            <h2 class="text-white font-semibold text-2xl">
                                Keuntungan
                                <br>
                                <h5 class="text-white  ">(Sampai Hari Ini)</h5>
                            </h2>

                            <h1 class="text-white  py-2 text-2xl">
                                <span>Rp. </span><?php echo number_format($totalKeuntunganTotal, 0 , ',' , '.');  ?>
                            </h1>
                        </div>
                    ...
                    <div class="flex items-center justify-between">
                        <div class="widget-label">
                            <h2 class="text-white font-semibold text-2xl">
                                BBM Terjual
                                <br>
                                <h5 class="text-white  ">(Per Hari Ini)</h5>
                            </h2>
        ...
        <br>
        <!-- Pie -->
        <div class="w-full h-full grid gap-2 grid-cols-1 md:grid-cols-3 mb-3 py-1">
            <div
                class="w-full rounded-2xl shadow-2xl  md:w-auto h-full col-span-2 flex flex-col items-center bg-white justify-center">
                <h2 class="py-1 font-semibold text-3xl">Penjualan BBM Dalam Liter</h2>
                <div class="w-full h-96 py-5  ">
                    <canvas class="w-full h-full" id="pieChart"></canvas>
                </div>
            </div>

            <div class="w-full md:w-auto px-3 h-full">
                <div
                    class="border-black/12.5 w-full shadow-xl dark:bg-slate-850 dark:shadow-dark-xl relative flex min-w-0 flex-col break-words rounded-2xl border-0 border-solid bg-white bg-clip-border">
                    <div class="p-4 pb-0 rounded-t-4">
                        <h6 class="mb-5 text-2xl">Informasi Penjualan BBM </h6>
                        <hr>
                    </div>
                    <div class="flex-auto py-8 p-4">
                        <ul class="flex flex-col pl-0 mb-0 rounded-lg">
                            <li
                                class="relative flex justify-between py-2 pr-4 mb-2 border-0 rounded-t-lg rounded-xl text-inherit">
                                <div class="flex items-center">
                                    <div class="inline-block w-12 h-12 mr-4 text-center text-black bg-center from-blue-500 to-violet-500 shadow-sm fill-current stroke-none
               bg-gradient-to-tl  dark:bg-gradient-to-tl dark:from-slate-750 dark:to-gray-850 rounded-2xl">
                                        <img class="w-9 h-9  items-center justify-center align-middle ml-1 mt-1"
                                            src="../img/offer.png" alt="">
                                    </div>
                                    <div class="flex  flex-col">
                                        <h6 class="mb-1 text-lg leading-normal text-slate-700  dark:text-black">BBM
                                            Terlaris</h6>
                                        <span
                                            class="text-xs leading-tight dark:text-black/80"><?php echo $jenisBBMFavorit ?>
                                            <span class="font-semibold"><?php echo $totalLitersTerjual ?>
                                                Liter</span></span>
                                    </div>
                                </div>
                                <div class="flex">
                                    <button
                                        class="group ease-in leading-pro text-xs rounded-3.5xl p-1.2 h-6.5 w-6.5 mx-0 my-auto inline-block cursor-pointer border-0 bg-transparent text-center align-middle font-bold text-slate-700 shadow-none transition-all dark:text-black"><i
                            ...
                            <li class="relative flex justify-between py-2 pr-4 mb-2 border-0 rounded-xl text-inherit">
                                <div class="flex items-center">
                                    <div
                                        class="inline-block w-12 h-12 mr-4 text-center text-black bg-center shadow-sm fill-current stroke-none bg-gradient-to-tl from-red-600 to-orange-600 dark:bg-gradient-to-tl dark:from-slate-750 dark:to-gray-850 rounded-2xl">
                                        <img class="w-9 h-9  items-center justify-center align-middle ml-1 mt-1"
                                            src="../img/check.png" alt="">
                                    </div>
                                    <div class="flex py-1 flex-col">
                                        <h6 class="mb-1 text-lg leading-normal  text-slate-700 dark:text-black">
                                            Penjualan Terbanyak</h6>
                                        <span
                                            class="text-xs leading-tight dark:text-black/80"><?php echo $penjualanPalingBanyak ?>
                                            <span class="font-semibold"><?php echo $totalPenjualanPalingBanyak ?>
                                                Liter</span></span>
                                    </div>
                                </div>
                                <div class="flex">
                            ...
                            <li
                                ...
                                <div class="flex items-center">
                                    <div
                                        class="inline-block w-12 h-12 mr-4 text-center text-black bg-center shadow-sm fill-current stroke-none bg-gradient-to-tl from-emerald-500 to-teal-400 dark:bg-gradient-to-tl dark:from-slate-750 dark:to-gray-850 rounded-2xl">
                                        <img class="w-9 h-9  items-center justify-center align-middle ml-1 mt-1"
                                            src="../img/discount.png" alt="">
                                    </div>
                                    <div class="flex flex-col">
                                        <h6 class="mb-1 text-lg leading-normal  text-slate-700 dark:text-black">
                                            Penjualan paling sedikit</h6>
                                        <span
                                            class="text-xs leading-tight dark:text-black/80"><?php echo $penjualanPalingSedikit; ?>
                                            <span class="font-semibold"> <?php echo  $totalPenjualanPalingSedikit; ?>
                                                Liter</span></span>
                                    </div>
                                </div>
                                <div class="flex">
                            ...
                            <li
                                ...
                                <div class="flex items-center">
                                    <div
                                        class="inline-block w-12 h-12 mr-4 text-center text-black bg-center shadow-sm fill-current stroke-none bg-gradient-to-tl from-orange-500 to-yellow-500 dark:bg-gradient-to-tl dark:from-slate-750 dark:to-gray-850 rounded-2xl">
                                        <img class="w-9 h-9  items-center justify-center align-middle ml-1 mt-1"
                                            src="../img/only-today.png" alt="">
                                    </div>
                                    <div class="flex flex-col">
                                        <h6 class="mb-1 text-lg leading-normal  text-slate-700 dark:text-black">Jumlah
                                            Pengisian (hari ini)</h6>
                                        <span class="text-xs leading-tight dark:text-black/80"><span
                                                class="font-semibold"> <?php echo $totalBbmTerjualHariIni; ?> Liter
                                            </span></span>
                                    </div>
                                </div>
                                <div class="flex">
        ...
        <div class="container mx-auto p-2">
            <div class="w-full px-5 py-5 bg-white shadow-xl rounded-2xl">
                <h2 class="text-2xl font-semibold mb-4">Keuntungan Perbulan</h2>
                <div class="max-h-96">
    ...
    <script src="../carouseldash.js"></script>
</body>
